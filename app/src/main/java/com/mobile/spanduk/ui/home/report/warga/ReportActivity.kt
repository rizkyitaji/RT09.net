package com.mobile.spanduk.ui.home.report.warga
...
import com.mobile.spanduk.databinding.ActivityReportBinding
...
class ReportActivity : AppCompatActivity() {
    ...
    private lateinit var bind: ActivityReportBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        bind = ActivityReportBinding.inflate(layoutInflater)
        ...
        if (net.isOnline()) {
            mDatabase = ref.orderByChild("type").equalTo("Aduan")

            mDatabase.addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    dataList.clear()
                    for (dataSnapshot in snapshot.children) {
                        dataList.add(dataSnapshot.getValue(Report::class.java)!!)
                    }
                    showData()
                }

                override fun onCancelled(error: DatabaseError) {
                    Toast.makeText(this@ReportActivity, ""+error.message, Toast.LENGTH_SHORT).show()
                }
            })
